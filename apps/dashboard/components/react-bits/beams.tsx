...
function extendMaterial<T extends THREE.Material = THREE.Material>(
  ...
  for (const [include, code] of Object.entries(config.vertex ?? {})) {
    vertexShader = vertexShader.replace(include, [include, code].join("\n"))
  }
  for (const [include, code] of Object.entries(config.fragment ?? {})) {
    fragmentShader = fragmentShader.replace(include, [include, code].join("\n"))
  }
...
function createStackedPlanesBufferGeometry(
  ...
  for (let planeIndex = 0; planeIndex < count; planeIndex += 1) {
    const xOffset = baseOffset + planeIndex * (width + spacing)
    const uvXOffset = Math.random() * 300
    const uvYOffset = Math.random() * 300

    for (
      let segmentIndex = 0;
      segmentIndex <= heightSegments;
      segmentIndex += 1
    ) {
...
const DirectionalLight: FC<{
  ...
  useEffect(() => {
    ...
    const camera = light.current.shadow.camera as THREE.Camera & {
      top: number
      bottom: number
      left: number
      right: number
