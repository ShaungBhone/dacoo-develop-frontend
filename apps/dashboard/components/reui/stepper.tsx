...
function StepperTrigger({
  ...
  const handleButtonRef = useCallback((node: Element | null) => {
    setButtonNode(node instanceof HTMLButtonElement ? node : null)
  }, [])
  useEffect(() => {
    if (!buttonNode) return

    return registerTrigger(buttonNode)
  }, [buttonNode, registerTrigger])

  // Find our index among triggers for navigation
  const myIdx = buttonNode ? triggerNodes.indexOf(buttonNode) : -1
