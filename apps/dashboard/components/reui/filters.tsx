...
function FilterInput<T = unknown>({
  ...
  return (
    <InputGroup
      className={cn(
        "w-48 shadow-none has-[[data-slot=input-group-control]:focus-visible]:border-input has-[[data-slot=input-group-control]:focus-visible]:ring-0 focus-within:ring-0",
        context.size == "sm" && "h-7!",
        context.size == "lg" && "h-9!",
        className
      )}
    >
      ...
      <InputGroupInput
        ...
        className={cn(
          "px-2.5 shadow-none border-0 ring-0 focus:ring-0 focus-visible:ring-0 focus:outline-none focus-visible:outline-none",
      ...
      {(isFocused || hasValue) && (
        <InputGroupAddon align="inline-end" className="pr-2.5 pl-0">
          <Kbd className="h-4 min-w-4 px-1 text-[10px] shadow-none text-muted-foreground/80">
  ...
  if (field.type === "text") {
    return (
      <FilterInput
        ...
        field={field}
        className={cn("w-48", field.className)}
        autoFocus={autoFocus}
      />
    )
  }
...
export function Filters<T = unknown>({
  ...
  const [draftTextValues, setDraftTextValues] = useState<Record<string, T[]>>(
    {}
  )
          ...
          return (
            ...
            >
              ...
              <FilterValueSelector<T>
                field={field}
                values={
                  deferTextInput && field.type === "text"
                    ? (draftTextValues[filter.id] ?? filter.values)
                    : filter.values
                }
                operator={filter.operator}
                onChange={(values) => {
                  if (deferTextInput && field.type === "text") {
                    setDraftTextValues((current) => ({
                      ...current,
                      [filter.id]: values,
                    }))
                    return
                ...
                onCommit={() => {
                  if (!deferTextInput || field.type !== "text") return

                  const values = draftTextValues[filter.id]
                  if (!values) return
