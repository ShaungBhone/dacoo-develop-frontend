...
function getDataGridBatchCellEdit<TData extends object>(
  column: Parameters<typeof getDataGridWritableCellEdit<TData>>[0],
  row?: TData
): DataGridColumnCellEdit<TData> | null {
  const cellEdit = getDataGridWritableCellEdit<TData>(column, row)
  return cellEdit?.batchEditable === false ? null : cellEdit
}
  ...
  for (const bound of bounds) {
    for (let r = bound.minRowIndex; r <= bound.maxRowIndex; r++) {
      ...
      for (let c = bound.minColumnIndex; c <= bound.maxColumnIndex; c++) {
        const column = columns[c]
        if (!column || column.columnDef.enableCellSelection === false) continue
        const cellEdit = getDataGridBatchCellEdit<TData>(
          column,
          row.original
        )
...
function buildDataGridPasteDetails<TData extends object>(
  ...
  for (let r = 0; r < targetRows.length; r++) {
    ...
    for (let c = 0; c < columnCount; c++) {
      ...
      if (!column) continue
      const raw = tiled[r]?.[c] ?? ""
      const cellEdit = getDataGridBatchCellEdit<TData>(column, row.original)
      if (!cellEdit) {
        rejected.push({
          rowId: row.id,
          columnId: column.id,
...
function startDataGridFillSession<TData extends object>(options: {
  ...
  const isFillableColumn = (column: (typeof columns)[number]) =>
    ...
    !!getDataGridBatchCellEdit<TData>(column)
  ...
  const applyPreview = () => {
    ...
    for (const { row } of targets.rowEntries) {
      ...
      for (const { column } of targets.columnEntries) {
        if (!getDataGridBatchCellEdit<TData>(column, row.original)) continue
  ...
  const finish = (commit: boolean) => {
    ...
    if (extension.axis === "row") {
      for (let offset = 0; offset < targets.rowEntries.length; offset++) {
        ...
        for (const { column } of targets.columnEntries) {
          if (!getDataGridBatchCellEdit<TData>(column, targetRow.original)) {
            continue
          }
          changes.push({
            rowId: targetRow.id,
      ...
      for (const { row } of targets.rowEntries) {
        ...
        for (const {
          ...
          const targetEdit = getDataGridBatchCellEdit<TData>(
            targetColumn,
            row.original
          )
  ...
  useEffect(() => {
    ...
    const requestCellEdit = (initialText?: string): boolean => {
      ...
      if (cellEdit && onCellEditRequest) {
        const cellElement = viewport.querySelector<HTMLTableCellElement>(
