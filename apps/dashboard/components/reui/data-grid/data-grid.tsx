...
/* eslint-disable react-hooks/refs, react-hooks/use-memo -- ReUI deliberately keeps mutable latest-value refs behind stable context getters so TanStack's v9 table wrapper does not republish the entire grid during resize and selection gestures. */
...
export interface DataGridColumnCellEdit<TData> {
  ...
  /**
   * Keep direct editing available while excluding this column from clipboard,
   * fill, cut, and clear batches. Reference and image fields need a picker or
   * upload flow and must never guess a typed value from clipboard text.
   */
...
export interface DataGridColumnMeta<TData> {
   ...
   */
  fillWidth?: boolean
  /** Source attribute metadata for derived record-table column instances. */
...
export interface DataGridCellEditRequest<TData> {
  ...
  row: TData
  previousValue: unknown
  /** The live cell anchor used by consumer-owned typed editor overlays. */
...
function DataGridProvider<TData extends object>({
  ...
  const autoSize = useMemo(
    ...
    // The stable store is the lifetime of this controller even though the
    // current wrapper is read through tableRef.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [table.store]
  )
