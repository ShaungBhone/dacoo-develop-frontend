...
function DataGridTableBase({ children }: { children: ReactNode }) {
  ...
  const columnSizeVars = useMemo(() => {
    ...
    return colSizes
    // `table` is a new wrapper on every TanStack v9 state change; the listed
    // slices are the stable signals that actually affect these CSS variables.
      ...
      className={cn(
        ...
        props.tableLayout?.columnsPinnable &&
          ...
          cn(
            ...
            "[&[data-pinned=end]:last-child_div.cursor-col-resize:last-child]:opacity-0 [&[data-pinned=end][data-last-col=end]]:shadow-[inset_1px_0_0_0_var(--border)]",
            !props.tableLayout?.cellBorder &&
    ...
    <td
      ...
      className={cn(
        ...
        props.tableLayout?.columnsPinnable &&
          column.getCanPin() &&
          cn(
            "data-pinned:isolate data-pinned:bg-background [&[data-pinned=end][data-last-col=end]]:shadow-[inset_1px_0_0_0_var(--border)]",
            !props.tableLayout?.cellBorder &&
              "[&[data-pinned=start][data-last-col=start]]:shadow-[inset_-1px_0_0_0_var(--border)]"
          ),
        column.getIndex() === 0 ||
...
function DataGridTableRowPin<TData extends object>({
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex size-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:text-foreground",
