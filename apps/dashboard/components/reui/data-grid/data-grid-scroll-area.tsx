...
const SCROLLBAR_CLASSNAME =
  "flex touch-none bg-muted/70 p-px transition-colors select-none data-[orientation=horizontal]:h-2.5 data-[orientation=horizontal]:flex-col data-[orientation=horizontal]:border-t data-[orientation=horizontal]:border-border data-[orientation=vertical]:h-full data-[orientation=vertical]:w-2.5 data-[orientation=vertical]:border-s data-[orientation=vertical]:border-border"

const SCROLLBAR_THUMB_CLASSNAME =
  "relative flex-1 cursor-grab rounded-full bg-muted-foreground/65 hover:bg-muted-foreground active:cursor-grabbing"
...
function DataGridScrollArea({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      scheduleSync()
      const footerViewport = footerViewportRef.current
      if (footerViewport) footerViewport.scrollLeft = viewport.scrollLeft
    }
    handleScroll()
    ...
    <div ref={containerRef} className="relative flex min-h-0 flex-1 flex-col">
      ...
      {footer && (
        <div
          ...
          data-slot="data-grid-external-footer"
          className="shrink-0 overflow-x-hidden border-t border-border bg-background"
        >
          {footer}
        </div>
      )}
